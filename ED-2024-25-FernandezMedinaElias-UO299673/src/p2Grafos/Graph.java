...
//		"Path could not be obtained: " + target + " is not present in the current graph.");
	...
	public boolean isSinkNode(T node) { // un nodo es sumidero si únicamente hay caminos que ENTRAN en él
	...
	public boolean isFontNode(T node) { //un nodo es fuente si únicamente hay caminos que SALEN de él
		int nodePos = getNode(node);
		for(int i=0; i<numNodes; i++) {
			if(edges[i][nodePos]) {
				return false;
	...
	public boolean isAislatedNode(T node) { //un nodo es asilado si no tiene ningun tipo de conexion
		int nodePos = getNode(node);
		for(int i=0; i<numNodes; i++) {
			if(edges[nodePos][i] || edges[i][nodePos]) {
				return false;
			}
		}
		return true;
	...
	public T getCentreNode() { //se averigua si un nodo es el centro  cogiendo el maximo de cada columna verticales de afloyd y agrupandolas en un vector, luego de ese vector se coge el mínimo
		double[] maxWeight = new double[numNodes];
		double currentWeight;
		for(int i=0; i<numNodes; i++) {
			 currentWeight = -1;
			for(int j=0; j<numNodes; j++) {
				if(aFloyd[j][i]>currentWeight) {
					maxWeight[i]=aFloyd[j][i];
					currentWeight = aFloyd[j][i];
				}
			}
		}
	...
	private boolean isNodeConected(T node) {
		int nodePos = getNode(node);
		for(int i=0; i<numNodes; i++) {
			if(aFloyd[nodePos][i]==Double.POSITIVE_INFINITY) {
				return false;
			}
		}
		return true;
	}
	public boolean isGraphConnected() { //un grafo es conexo si todos sus nodos son conexos y se puede llegar a cualquier nodo desde cualquier otro
		for(int i=0; i<numNodes; i++) {
			if(!isNodeConected(nodes[i])) {
				return false;
			}
		}
		return true;
	...
	public double graphDensity() { // d=(2m/n(n-1))   --> 0=vertices aislados 1=grafo completo
		double numerator = 2*countEdges();
		double denominator= numNodes*(numNodes-1);
		double result =  numerator/denominator;
		double roundOff = (double)Math.round(result*100)/100;
		if(countEdges()>getMaxNumberOfEdges()) {
			roundOff=1;
		}
		return roundOff;
	}
	public int getMaxNumberOfEdges() {
		...
		if(hasLoop) {
			return (int) ((Math.pow(numNodes, 2)+numNodes)/2);
		}else {
			return (int) ((Math.pow(numNodes, 2)-numNodes)/2);
		}
	}
private int maxCost(boolean[] visited, double[] vectorD) {
		int nodePos = -1;
		double maximum = -1;
		for(int i=0; i<numNodes; i++) {
			if(!visited[i] && vectorD[i]>maximum) {
				maximum = vectorD[i];
				nodePos=i;
			}
		}
	...
	public DijkstraDataClass maxDijkstra(T origen) {
		int nodePos = getNode(origen);
		DijkstraDataClass d = new DijkstraDataClass(numNodes, nodePos);
		double[] vectorD = d.getdDijkstra();
		int[] vectorP = d.getpDijkstra();
		boolean[] visited = new boolean[numNodes];
		for(int i=0; i<numNodes; i++) {
			vectorD[i]=-1;
			vectorP[i]=-1;
		...
		vectorD[nodePos]=0;
		for(int i=0;i<numNodes; i++) {
			int currentNode = maxCost(visited, vectorD);
			if(currentNode==-1) {
				break;
			}
			visited[currentNode]=true;
			for(int neighbor = 0; neighbor<numNodes; neighbor++) {
				if(edges[currentNode][neighbor]) {
					if(vectorD[neighbor]<vectorD[currentNode]+weights[currentNode][neighbor]) {
						vectorD[neighbor]=vectorD[currentNode]+weights[currentNode][neighbor];
						vectorP[neighbor]=currentNode;
					}
				}
			}
